...
(function (global) {
  ...
  var CONFIG = {
    ...
    backend: 'gsheet',
    // Seed so the queue feels alive on day one. MUST match SEED in backend/Code.gs.
    seedCount: 2847,
    gsheet: {
  ...
  var Local = (function () {
    ...
    function init(d) {
      if (!d.people) { d.people = {}; }      // code -> { email, code, ref, joinedAt, order }
      if (!d.byEmail) { d.byEmail = {}; }     // email -> code
    ...
    function crewOf(d, code) {
      ...
      return list.sort(function (a, b) { return a.order - b.order; })
                 .map(function (p) { return p.name || (p.email.charAt(0) + '•••' + p.email.slice(p.email.indexOf('@'))); });
    }

    return {
      join: function (email, ref, name) {
        var d = init(db());
        email = email.trim().toLowerCase();
        var existing = d.byEmail[email];
        if (existing && d.people[existing]) {
          var st = positionOf(d, existing);
          return Promise.resolve({ email: email, code: existing, position: st.position, refCount: st.refCount, crew: crewOf(d, existing), returning: true });
        }
        var code = makeCode();
        while (d.people[code]) { code = makeCode(); }
        d.people[code] = { email: email, name: (name || '').trim(), code: code, ref: (ref && d.people[ref]) ? ref : null, order: d.next++ };
        d.byEmail[email] = code;
        save(d);
        var s = positionOf(d, code);
        return Promise.resolve({ email: email, code: code, position: s.position, refCount: s.refCount, crew: crewOf(d, code), returning: false });
      },
      ...
      invite: function (code, friendEmail) {
        // Records intent locally. A real backend would send the email here.
        var d = init(db());
        d.invites = d.invites || [];
        d.invites.push({ from: code, to: friendEmail.trim().toLowerCase() });
        save(d);
  ...
  var Sheet = {
    ...
    join: function (email, ref, name) {
      email = email.trim().toLowerCase();
      return Sheet._call({ action: 'join', email: email, ref: ref || '', name: name || '' })
        .then(function (d) {
          return { email: email, code: d.code, position: d.position, refCount: d.refCount || 0, crew: d.crew || [], returning: !!d.returning };
        });
    },
    status: function (code) {
    ...
    invite: function (code, friendEmail) {
      return Sheet._call({ action: 'invite', code: code, email: friendEmail.trim().toLowerCase() })
  ...
  global.Waitlist = {
    ...
    me: recall,
    join: function (email, name) {
      return impl.join(email, getRefFromUrl(), name).then(function (res) {
        var me = { email: res.email, name: (name || '').trim(), code: res.code, refLink: refLinkFor(res.code) };
        remember(me);
        return res;
      });
    },
    status: function (code) { return impl.status(code); },
    invite: function (code, friendEmail) { return impl.invite(code, friendEmail); },
